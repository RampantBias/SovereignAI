...
func writeDebugContext(
	input agentcontract.Input,
	artifacts []loadedArtifact,
	round int,
	request inference.ChatRequest,
	rawHistory []inference.Message,
) {
	path := filepath.Join(input.StagingPath, "debug-context.json")
	if err := saveDebugContext(path, input, artifacts, round, request, rawHistory); err != nil {
...
func saveDebugContext(
	...
	snapshot := struct {
		...
		RawHistory []inference.Message   `json:"rawInferenceHistory,omitempty"`
	}{Input: input, Round: round, Request: request, RawHistory: rawHistory}
